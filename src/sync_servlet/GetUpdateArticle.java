...
public class GetUpdateArticle extends HttpServlet {
	...
	protected void perform(HttpServletRequest request, HttpServletResponse response) throws ServletException,
		...
		try {
			...
			// JSONを配列へ変換(ブラウザのリスト）
			JsonArticle[] json_list = JSON.decode(strJson, JsonArticle[].class);
			System.out.println(json_list.length);
			...
			if (json_list.length != 0) {
				...
				for (ArticleBean art : article_list) {
					...
					if (old_json_map.containsKey(art.getArticle_id())) {
						// 日付を比較する
						// サーバ＞ブラウザ
						if (article_map.get(art.getArticle_id()).getModified() > old_json_map.get(art.getArticle_id()).getModified()) {
							// 存在しなければ新規追加（更新必要）
		...
		System.out.println(update_list.size());
